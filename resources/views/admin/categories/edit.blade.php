...
<form method="POST" action="{{ route('admin.categories.update', $category) }}">
  ...
  <div class="mb-3">
    ...
    <select name="type" class="form-select @error('type') is-invalid @enderror" required>
      <option value="makanan" @selected(old('type', $category->type)=='makanan')>Makanan</option>
      <option value="minuman" @selected(old('type', $category->type)=='minuman')>Minuman</option>
      <option value="Cake" @selected(old('type', $category->type)=='Cake')>Cake</option>
    </select>
